...
(function($) {
    ...
    $.fn.combobox = function(suggestions, config) {
        config = $.extend({ btnCls: 'combobox-btn' }, config);
        
        if(!optionsContainer) {
            optionsContainer = $('<div class="comboboxDropdown"><ul></ul></div>').appendTo($('body'));
        ...
        $(this).each(function(i) {
            ...
            var additionalHeight =3;
            var button = $('<em class="'+config.btnCls+'"><i></i><b></b></em>').insertAfter(textBox).click(function() {
                textBox.focus();
            });
           // textBox.width(textBox.width() - button.width());
            ...
            function show(filter) {
                ...
                var loc = { left: textBox.offset().left, top: textBox.offset().top + textBox.height() + 1, width: textBox.outerWidth() + button.outerWidth() }
                optionsContainer.find("ul").html(html)
                optionsContainer.css(loc);

                //decide which option is currently selected
                selIndex = 0;
                var found = false;
                var options = optionsContainer.find("ul").children('li').each(function(i) {
                    if (found) return;
            ...
            function godown() {
                if (showing) {
                    var options = optionsContainer.find("ul").children('li');
                    var n = options.size();
                    if (!n)
                        return;
                    selIndex++;
            ...
            function hide() {
                if (showing) {
                   optionsContainer.hide().find("ul").children('li').each(function() { $(this).remove(); });
                    showing = false;
                }
            }
